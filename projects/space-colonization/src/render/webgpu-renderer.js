...
import { CAMERA_UNIFORM_BYTES } from "./render-layout.js";
...
export class WebGpuGalaxyRenderer {
  ...
  /** @param {{ starBuffer: ArrayBuffer, starCount: number, pickTable: (import("../core/body-ref.js").BodyRef|null)[] }} snapshot */
  update(snapshot) {
  ...
  /** @param {{ viewProjection?: Float32Array, pointSize?: number }} camera */
  render(camera = {}) {
  ...
  async pick(x, y, camera) {
    ...
    const targetX = (((x - bounds.left) / bounds.width) * 2 - 1) * zoom * aspect;
    const targetY = (1 - (((y - bounds.top) / bounds.height) * 2)) * zoom;
    let closestIndex = -1;
    ...
    return { body, positionParsecs: [camera.positionParsecs[0] + this.starPositions[offset], camera.positionParsecs[1] + this.starPositions[offset + 1], camera.positionParsecs[2] + this.starPositions[offset + 2]] };
  }
